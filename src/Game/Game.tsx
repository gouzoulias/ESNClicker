...
export const Game = ({ children }: React.PropsWithChildren) => {
  ...
  const buyUpgrade = useCallback(
    (upgrade: Upgrade) => {
      const upgradeInfo = UpgradeInfos[upgrade];
      ...
      const canAffordMoney = !hasMoneyPrice || money >= upgradeInfo.price!;
      const canAffordCodeLines = !hasCodeLinePrice || codeLines >= upgradeInfo.priceInCodeLines!;
      const canAfford = canAffordMoney && canAffordCodeLines;

      if (canAfford) {
        if (hasMoneyPrice) {
          setMoney((prevState) => prevState - upgradeInfo.price!);
        }
        if (hasCodeLinePrice) {
          setCodeLines((prevState) => prevState - upgradeInfo.priceInCodeLines!);
        }
        ...
        switch (upgrade) {
          ...
          case Upgrade.Linter:
            ...
            break;
          case Upgrade.KeyboardShortcuts:
            setManualProductivity((prevState) => prevState * 1.5);
            break;
    ...
    [money, codeLines, updateDevTeam, updatePOTeam],
  );
